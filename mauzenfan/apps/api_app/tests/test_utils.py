...
class WeatherServiceTests(TestCase):
    ...
    def test_get_weather_forecast_success(self, MockOWM):
        ...
        MockOWM.return_value = mock_owm_instance # OWM(api_app_KEY) returns our mock
        ...
        # Temporarily set WEATHER_api_app_KEY in settings for the service to attempt initialization
        # and use our MockOWM which returns the mock_owm_instance.
        with self.settings(WEATHER_api_app_KEY='fakekey'):
    ...
    def test_get_weather_forecast_api_app_error(self, MockOWM):
        mock_owm_instance = MagicMock()
        mock_mgr = MagicMock()
        ...
        with self.settings(WEATHER_api_app_KEY='fakekey_error'):
            with patch('api_app.weather_service.owm_client', mock_owm_instance):
                result = weather_service.get_weather_forecast(10.0, 10.0)
        self.assertIsNone(result)
